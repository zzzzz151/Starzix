...
private:
    ...
    constexpr i32 search(ThreadData* td, i32 depth, i32 ply)
    {
        ...
        if (shouldStop(td)) return evaluate(td->pos);
        ...
        MovePicker movePicker = MovePicker(td->pos);
        Move move;
        ...
        while ((move = movePicker.nextLegal(td->pos)) != MOVE_NONE)
        {
            ...
            const i32 score = td->pos.isDrawIgnoringStalemate(ply + 1)
                            ? 0
        ...
        if (legalMovesSeen == 0)
        {
            ...
            return td->pos.inCheck() ? -INF + ply : 0;
        }
