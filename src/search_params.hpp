...
TunableParam<float> evalMaterialScaleMin = TunableParam<float>(0.7, 0.5, 1.0, 0.1);
...
TunableParam<double> aspDeltaMultiplier = TunableParam<double>(1.37, 1.2, 2.0, 0.1);
...
TunableParam<i32> rfpMultiplier = TunableParam<i32>(65, 40, 130, 10);
...
TunableParam<float> probcutImprovingPercentage = TunableParam<float>(0.28, 0.25, 0.75, 0.1);
...
TunableParam<float> lmpMultiplier = TunableParam<float>(1.08, 0.5, 1.5, 0.1);

// FP (Futility pruning)
TunableParam<i32> fpMaxDepth   = TunableParam<i32>(7, 6, 10, 1);
TunableParam<i32> fpBase       = TunableParam<i32>(160, 40, 260, 20);
TunableParam<i32> fpMultiplier = TunableParam<i32>(157, 40, 260, 20);

// SEE pruning
TunableParam<i32> seePruningMaxDepth = TunableParam<i32>(8, 7, 11, 1);
TunableParam<i32> seeQuietThreshold  = TunableParam<i32>(-55, -161, -1, 20);
TunableParam<i32> seeNoisyThreshold  = TunableParam<i32>(-124, -161, -1, 20);

// SE (Singular extensions)
TunableParam<i32> singularMinDepth      = TunableParam<i32>(6, 6, 10, 1);
TunableParam<i32> singularDepthMargin   = TunableParam<i32>(4, 1, 5, 1);
TunableParam<i32> doubleExtensionMargin = TunableParam<i32>(40, 2, 42, 10);
constexpr u8 DOUBLE_EXTENSIONS_MAX = 10;

// LMR (Late move reductions)
TunableParam<double> lmrBaseQuiet       = TunableParam<double>(0.735, 0.4, 1.2, 0.1);
TunableParam<double> lmrMultiplierQuiet = TunableParam<double>(0.5, 0.2, 0.8, 0.1);
TunableParam<double> lmrBaseNoisy       = TunableParam<double>(0.685, 0.4, 1.2, 0.1);
TunableParam<double> lmrMultiplierNoisy = TunableParam<double>(0.33, 0.2, 0.8, 0.1);
TunableParam<i32>    lmrQuietHistoryDiv = TunableParam<i32>(29757, 8192, 32768, 2048);
TunableParam<i32>    lmrNoisyHistoryDiv = TunableParam<i32>(3499, 1024, 16384, 1024);
...
TunableParam<i32> historyBonusOffset     = TunableParam<i32>(38, 0, 500, 100);
TunableParam<i32> historyBonusMax        = TunableParam<i32>(1730, 500, 2500, 200);

// History malus
TunableParam<i32> historyMalusMultiplier = TunableParam<i32>(272, 50, 600, 50);
TunableParam<i32> historyMalusOffset     = TunableParam<i32>(12, 0, 500, 100);
TunableParam<i32> historyMalusMax        = TunableParam<i32>(1381, 500, 2500, 200);

// History weights
TunableParam<float> mainHistoryWeight  = TunableParam<float>(1.066, 0.2, 4.0, 0.2);
TunableParam<float> contHist1PlyWeight = TunableParam<float>(0.88, 0.2, 4.0, 0.2);
